...
int main(int argc, char *argv[])
{
    // if (argc != 4) {
    if (argc < 4) {
        std::cout << "Usage: ./CFDMiner_Graph_Probability minsupp maxsize csv_files_folder..." << std::endl;
        ...
        for (const std::string& filename : partition_filenames) {
            ...
            *db = temp_db;
            ...
            for (const ccfd& rule : new_ccfd_list) {
                ...
                // Try to use `1` instead of `rule.supp`
                unified_ccfd_list.push_back(ccfd{new_lhs, new_rhs, 1, 0});
            }
